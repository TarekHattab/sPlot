...
sourceCpp("BetaJtu_RcppParallel.cpp")
sourceCpp("BetaJtu_OpenMP.cpp")
sourceCpp("hcr.C.cpp")
sourceCpp("cast_binary.cpp")

# R wrapper for BetaJtu C++ function
BigBetaJtu <- function(bigMat){
    zeros <- big.matrix(nrow = nrow(bigMat),
                        ncol = nrow(bigMat),
                        init = 0,
                        type = "double",
			shared=FALSE,
			backingfile=paste("betaJtuMatrix_",i,sep=""),
			backingpath=getwd(),
			descriptorfile=paste("betaJtuMatrix_",i,".desc",sep=""))
    betaJtu(bigMat@address, zeros@address)
    return(zeros)
}

# get the 3rd axis for plot_data
bioclim_wrld<-as(bioclim_wrld,"SpatialPixelsDataFrame")
bioclim_wrld<-brick(bioclim_wrld)
plot_data@data[,c("pc1_val","pc2_val","pc3_val")]<-extract(bioclim_wrld,plot_data)[,c("pc1_val","pc2_val","pc3_val")]

# Setting resolution
res=50
###################
xx<-seq(min(plot_data@data$pc1_val,na.rm=T),max(plot_data@data$pc1_val,na.rm=T),length.out=res)
yy<-seq(min(plot_data@data$pc2_val,na.rm=T),max(plot_data@data$pc2_val,na.rm=T),length.out=res)
zz<-seq(min(plot_data@data$pc3_val,na.rm=T),max(plot_data@data$pc3_val,na.rm=T),length.out=res)
xyz<-expand.grid(xx,yy,zz)

#count the number of plots in each cube

count3d<-function(x){
xp<-plot_data@data[,c("pc1_val")]
yp<-plot_data@data[,c("pc2_val")]
zp<-plot_data@data[,c("pc3_val")]
xg<-xyz[x,1]
yg<-xyz[x,2]
zg<-xyz[x,3]
xres=abs(xx[1]-xx[2])
yres=abs(yy[1]-yy[2])
zres= abs(zz[1]-zz[2])
count<-length(which(xp > xg-(xres/2) & xp < xg+(xres/2) & yp > yg-(yres/2) & yp < yg+(yres/2) & zp > zg-(zres/2) & zp < zg+(zres/2)))
return(count)}

# If your are using windows 
#install_github('nathanvan/parallelsugar')
library(parallelsugar)
Plot_count<-parallelsugar::mclapply(X=1:nrow(xyz),FUN=count3d)
# if your are using Linux or Mac 
#library(parallel)
#Plot_count<-parallel::mclapply(X=1:nrow(xyz),FUN=count3d)

xyz[,"Plot_count"]<-unlist(Plot_count)
xyz<-xyz[-which(xyz[,4]==0),]
cutoff<-median(xyz[,4])
print(paste("Nb cells grid= ", nrow(xyz),"   cutoff= ",cutoff,sep=""))

# 3d plot
scale.col<-(xyz[,4]-min(xyz[,4]))/(max(xyz[,4])-min(xyz[,4]))
library(colorRamps)
colorFunction <- colorRamp(matlab.like(100))
zMatrix <- colorFunction(scale.col)
den.col <- rgb(zMatrix[,1], zMatrix[,2], zMatrix[,3], maxColorValue=255)
library(rgl)
rgl::open3d()
for(i in 1:nrow(xyz)){
moncube<-rgl::cube3d(col = den.col[i])
moncube$vb[1,] <- moncube$vb[1,]*abs(xx[1]-xx[2])
moncube$vb[2,] <- moncube$vb[2,]*abs(yy[1]-yy[2])
moncube$vb[3,] <- moncube$vb[3,]*abs(zz[1]-zz[2])
rgl::shade3d(rgl::translate3d( moncube, xyz[i,1],xyz[i,2],xyz[i,3]),alpha=0.1)}
#rgl::plot3d(plot_data@data[,c("pc1_val","pc2_val","pc3_val")], col=3, type ="s",r=.1,add=T)
					  
plot_data <- plot_data@data					  
tempZoneOut <- xyz[which(xyz[,4]>cutoff),c(1,2,3)]

# Set Nb repetation in HCR
repet<-100


plotToRemove<-as.list(rep(NA,repet))
  for (i in 1:nrow(tempZoneOut)){
  sel.plot <- which(plot_data$pc1_val > tempZoneOut[i,1]-(abs(xx[1]-xx[2])/2) & 
                    plot_data$pc1_val < tempZoneOut[i,1]+(abs(xx[1]-xx[2])/2) &
                    plot_data$pc2_val > tempZoneOut[i,2]-(abs(yy[1]-yy[2])/2) & 
                    plot_data$pc2_val < tempZoneOut[i,2]+(abs(yy[1]-yy[2])/2) &
 	                  plot_data$pc3_val > tempZoneOut[i,3]-(abs(zz[1]-zz[2])/2) & 
                    plot_data$pc3_val < tempZoneOut[i,3]+(abs(zz[1]-zz[2])/2))
  ...
  if (nrow(comm.data)>20000) {bigComMatrix <- as.big.matrix(comm.data,shared=FALSE,backingfile=paste("Matrix_",i,sep=""),backingpath=getwd(),descriptorfile=paste("Matrix_",i,".desc",sep="")) ; brayBalDist <- BigBetaJtu(bigComMatrix)}
  else { brayBalDist <- betaJtu_RcppParallel(comm.data); brayBalDist <- as.big.matrix(brayBalDist)}
